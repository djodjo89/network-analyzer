...
public class FichierExtracteur implements IFichierExtracteur {
  private int fromHexaToInt(String number) {
    ...
    while (valeur == - 1 && i < Hexa.values().length) {
      ...
      if (currentHexa.getValue().equals(rawValue.toUpperCase())) valeur = currentHexa.ordinal();
      i++;
    }
  ...
  private String fromHexaToBits(String number) {
    ...
    return fromDecimalToBits(valeur) + fromHexaToBits(number.substring(1));
  }

  private String fromDecimalToBits(int number) {
    if (number == 0) return "";
    int[] puissancesDeDeux = { 8, 4, 2, 1 };
    int plusGrandePuissance = Arrays
            .stream(puissancesDeDeux)
            .filter(value -> value <= number)
            .findFirst()
            .orElse(0);

    int nouvelleValeur = number - plusGrandePuissance;
      
    return plusGrandePuissance != 0 ? "1" : "0" +
           fromDecimalToBits(nouvelleValeur);
  }
  ...
  public List<ITrame> extraireTrames(String nomFichier) {
    try {
      ...
      System.out.println(fromHexaToBits("a"));
